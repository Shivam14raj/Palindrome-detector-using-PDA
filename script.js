...
hoverElements.forEach((element) => {
    element.addEventListener("mouseenter", () => {
        cursor.classList.add("hover"); // Adds enlarged effect
    });

    element.addEventListener("mouseleave", () => {
        cursor.classList.remove("hover"); // Resets size
    });
});

document.getElementById("checkPalindromeBtn").addEventListener("click", function() {
    window.location.href = "detector.html";  // Redirect to the 'detector.html' page
});
